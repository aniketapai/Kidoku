...
import '../../features/chat_coach/presentation/chat_coach_screen.dart';
...
import '../../features/reels/presentation/reel_library_screen.dart';
...
final GoRouter appRouter = GoRouter(
  ...
  routes: [
    ...
    // Pushed on top of the shell (not a shell branch) so it gets a back
    // button, reached by tapping a video from the Videos tab's browse list.
    GoRoute(
      path: AppRoutes.reelPlayer,
      builder: (context, state) =>
          ReelsScreen(args: state.extra! as ReelPlayerArgs),
    ),
    // Reached from the drawer on ProfileScreen rather than the bottom nav —
    // pushed on top of the shell (so they get a back button via the
    // Scaffold/AppBar below) instead of living in a persistent tab branch.
    GoRoute(
      path: AppRoutes.vocabulary,
      builder: (context, state) => Scaffold(
        appBar: AppBar(title: const Text('Vocabulary')),
        body: const VocabularyScreen(),
      ),
    ),
    GoRoute(
        ...
        StatefulShellBranch(
          routes: [
            GoRoute(
              ...
              builder: (context, state) => const ReelLibraryScreen(),
            ),
          ],
        ),
        StatefulShellBranch(
          routes: [
            GoRoute(
              path: AppRoutes.chatCoach,
              builder: (context, state) => const ChatCoachScreen(),
            ),
          ],
        ),
        StatefulShellBranch(

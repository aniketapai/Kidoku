...
// Hand-written (not @riverpod codegen): MyWordEntry wraps Drift-generated
...
class MyWordEntry {
  const MyWordEntry({required this.userWord, required this.entry});

  final UserWord userWord;
  final DictionaryEntry? entry;
}

/// Every saved/known word (see [AppDatabase.watchAllUserWords]) joined with
/// its dictionary entry — the My Words screen splits this into Saved/Known
/// sections client-side by [UserWord.status].
final myWordsProvider = StreamProvider.autoDispose<List<MyWordEntry>>((ref) async* {
